#!/usr/bin/env python3
"""
Monday Brief Generator — FAST TEST VERSION
Project 4: The First Cloud Schedule

Same logic as the real version (fetch GitHub trending repos + Hacker News top stories
— real public data, no fake data needed). Two differences:

1. Output goes to test/output/test-brief-<timestamp>.md (never overwrites real files)
2. Run Metadata says "Trigger: fast test schedule" instead of "Trigger: scheduled"
"""
...
def render_brief(github, hn, run_ts, out_path):
    ...
    lines.append("")
    lines.append(f"- Trigger: fast test schedule")
...
def main():
    ...
    out_file = out_dir / f"test-brief-{now.strftime('%Y-%m-%d-%H%M%S')}.md"
